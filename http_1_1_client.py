...
class HTTPClient:
    ...
    def get(self, url, headers=None, stream=False):
        ...
        if headers:
            for head in headers:
                headers_str += f"{head}: {headers[head]}\r\n"
    ...
    def __send_request(self, address, request, stream):
        ...
        if f'{address[0]}:{address[1]}' not in self.connection_pool:
            self.connection_pool[f'{address[0]}:{address[1]}'] = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        client_socket = self.connection_pool[f'{address[0]}:{address[1]}']
        ...
        while True:
            ...
            try:
                ...
                client_socket.settimeout(5)
                self.connection_pool[f'{address[0]}:{address[1]}'] = client_socket
                try:
