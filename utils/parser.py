...
def parse_resource(resource):
    ...
    for param in query:
        # Split the param into a key-value pair
        param = param.split('=')
        if (len(param) == 2):
            key, value = param
            params[key] = value
...
def parse_response_2(stream_id, raw_bytes):
    ...
    """
    Hint: raw_bytes format: b"<headers>"
            <headers>::= <header>*
                <header>::= <key>": "<value>"\r\n"
    ...
    for poop in lines:
        if poop.decode() == '':
            break
        poop1, poop2 = poop.decode().split(': ')
        headers[poop1.lower()] = poop2
...
def parse_response(client_socket, stream):
    ...
    """
    Hint: Response raw_bytes format: b"<HTTP-version> <status>\r\n<headers>\r\n<body>"
            <headers>::= <header>*
                <header>::= <key>": "<value>"\r\n"
    ...
    response.version = lines[0].decode().split(' ')[0]
    response.status = lines[0].decode().split(' ')[1:]
...
def parse_request(client_socket):
    ...
    raw_bytes = b""
    # Receiving bytes until the header is completely received
    while True: 
        try:
            recv_bytes = client_socket.recv(4096)
            if recv_bytes == b"":
    ...
    """
    Hint: Request raw_bytes format: b"<method> <resource> <HTTP-version>\r\n<headers>\r\n<body>"
            <resource>::= <path>[<query>]
            <headers>::= <header>*
                <header>::= <key>": "<value>"\r\n"
    ...
    request.method, url, request.version = lines[0].decode().split(' ')
    resource = parse_url(url)[2]
