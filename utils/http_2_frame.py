...
class Frame:
    ...
    def to_bytes(self):
        # TODO return the bytes formatted by length, type, flags, r, stream_id, payload
        ...
        # bytes = ? (this is builtin type, don't override)
        ret = struct.pack("!LBL", 
            self.length << 8 | self.type, 
            self.flags, 
            self.r << 31 | self.stream_id) + self.payload;
...
def bytes_to_frames(data):
    ...
    while len(data) > 0:
        ...
        # Hint: The frame header has 9 bytes, and the payload has 'length' bytes
            ...
            a, b, c = struct.unpack("!LBL", data[:9])
            typ3 = a & 0xff
            flags = b
            r_stream_id = c
            payload = data[9:9+length]
            frame = Frame(length=length, type=typ3, flags=flags, r=r_stream_id>>31, stream_id=r_stream_id&0x7fffffff, payload=payload)
            frames.append(frame)
            data = data[9+length:]
        else:
            break
